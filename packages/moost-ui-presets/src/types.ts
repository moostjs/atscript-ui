import type { PresetSnapshotWire } from "@atscript/ui-table";

export type { FilterCondition, FilterConditionType, PresetSnapshotWire } from "@atscript/ui-table";
 ...
 * id `${user}:${app}:${tableKey}`). Carries preferences that are inherently
 * per-table (preset pins / favorites); app-wide prefs live on `AppConfData`.
 */
 ...
 * `${user}:${app}`). Holds preferences that are inherently app-scoped —
