// Message-only finish — no data, no `end` action. One trivial input round-trip
...
import { finishWfWithMessage } from "@atscript/moost-wf";
...
export class WfFinishMessageDemoWorkflow {
  ...
  run(
    ...
    ctx.note = input.note;
    finishWfWithMessage("info", "Nothing to do here — the workflow just emitted a message.");
    return;
  }
}
