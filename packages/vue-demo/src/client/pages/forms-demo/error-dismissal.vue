    ...
    <div class="max-w-2xl mx-auto p-$l flex flex-col gap-$l">
      <header class="flex flex-col gap-$xs">
        ...
        <p class="text-callout text-current-muted m-0 mt-$xxs">
          Manual harness for the three AsForm error-handling behaviours: external leaf
          auto-dismiss on edit, the loading overlay, and the dismissable
          <code>__form</code> banner. Each button replaces the entire
          <code>errors</code> object reference so the form's identity watch fires.
        </p>
        <RouterLink
          to="/forms-demo"
