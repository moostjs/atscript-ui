...
| 3   | **Form data is wrapped: `{ value: domainData }`.** Pass it as `:form-data` or omit and let `createAsFormDef` produce one. Path helpers and composables unwrap automatically. When you supply a `submitValidator`, it receives the *unwrapped* domain data.                                                                                                          |
...
import {
  AsFieldShell, AsInput, AsNumber, AsDecimal, AsSelect, AsRadio, AsCheckbox,
  AsDate, AsDatetime, AsTime, AsParagraph, AsAction,
  AsObject, AsArray, AsUnion, AsTuple, AsRef,
} from "@atscript/vue-form";

// Composables
import {
  useAsForm, useAsField, useAsState, useAsArray, useAsUnion, useAsTuple,
...
// Factories + providers
import {
  createDefaultTypes, createAsFormDef, formatIndexedLabelParts,
  provideAsLocale, provideAsNestedSectionsStore,
} from "@atscript/vue-form";

// Types
import type {
  TAsComponentProps, TAsComponentEmits, TAsChangeType, TAsTypeComponents,
  TAsUnionContext, TFormState, TFormRule,
  UseAsFieldOptions, UseAsFieldReturn, UseAsFormOptions, UseAsFormReturn,
} from "@atscript/vue-form";
```
