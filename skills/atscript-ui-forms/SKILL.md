...
| 10  | **`@ui.form.action` on a regular input renders an inline link.** On a non-phantom input it appears as a link-styled button in the field footer and emits the same `action` event; in `<AsWfForm>` it resolves via `@AltAction()`. See [actions-refs.md](references/actions-refs.md).                                                                                                                                                                                                                                       |
