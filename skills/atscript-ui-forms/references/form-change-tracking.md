...
// Fetch-then-fill: assign whenever the request resolves. A CLEAN swap
// re-baselines the tracker onto the loaded row (0.1.134+) — no `v-if` gate.
watch(
  () => props.id,
  async () => {
...
| 13  | **A CLEAN container swap re-baselines (0.1.134+).** Replacing `formData.value` by identity while `isDirty === false` re-captures the baseline onto the new value — so a form mounted over `createAsFormDef`'s seeded defaults and filled later is NOT born dirty, and no `v-if` gate is needed. A swap on a DIRTY form keeps the old baseline (the unsaved edits would be lost silently): call `rebase()` to discard them or `rebaseOnto()` to keep them on top of the new row. |
