...
`@atscript/vue-aooth` ships four pre-built custom field components used across [Aooth](https://aooth.moost.org) auth flows. All implement the `TAsComponentProps` contract and drop into `<AsForm :components>` (or `<AsWfForm :components>`) via `@ui.form.component`. None are auto-resolved — register each by name; the `.as` annotation value is just the map key.
...
Full prop shapes + worked examples: docs API reference at https://ui.atscript.dev/api/vue-aooth and the narrative at https://ui.atscript.dev/forms/aooth-components.
...
| #   | Rule                                                                                                                                                                                                                                                       |
| --- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| 1   | **Field-level `@ui.form.fn.*` callbacks are `(v, data, ctx, entry)`.** The two-arg `(_, ctx) => …` shape is wrong — arg 2 is `data`. Only **form-level** keys (`@ui.form.fn.title` / `submit.text` / `submit.disabled`) use `(data, ctx)`.                 |
...
```ts
import { AsConsentArray, AsCopy, AsPasswordRules, AsQrCode } from "@atscript/vue-aooth";
// per-component subpaths exist too: @atscript/vue-aooth/as-qr-code, …
```
