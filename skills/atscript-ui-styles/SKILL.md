...
description: >-
  ...
  without UnoCSS via `@atscript/ui-styles/css/{all,form,table,wf,aooth}`; or when
...
| 2   | **`asPresetVunor()` already injects `allShortcuts`.** Spread individual shortcut groups (`formShortcuts`, `tableShortcuts`, `wfShortcuts`, `aoothShortcuts`, `commonShortcuts`) only when building a subset preset.                                                                                                                                                                                                                |
...
| 8   | **Icon customization order: `iconOverrides` → your own `presetIcons` under a different prefix → `icons: false`.** `createIconsLoader(opts)` IS exported (0.1.133+) but is Node-only (reads/writes `.icons/` + an on-disk Iconify cache) — never import it from browser code. There is no `asIconsPreset` export; the baked collection is registered inside `asPresetVunor()`.                                                      |
...
| Icons               | [icons.md](references/icons.md)                                            | Default semantic alias set (what ships out of the box), `iconOverrides` shape + the `currentColor` / `1em` rules, adding new icons via `@unocss/preset-icons` (separate prefix), `bakedIcons` constant; running a SINGLE `presetIcons` instance (`asPresetVunor({ icons: false })` + `bakedIcons` / `createIconsLoader` + `defaultAsIconAliases`)                                                                                                                                                                                                                                               |
| Shortcut tree       | [shortcuts.md](references/shortcuts.md)                                    | `as-*` naming convention (variant suffix), four shortcut groups (form / table / wf / common), **overriding a built-in `as-*` shortcut** (append-not-replace merge, write only the one variant key, `!`-important + font numeric order, `content-['']` to blank a pseudo-element), extending with `mergeVunorShortcuts` + `defineShortcuts`, composing from vunor primitives for inherited theming, `excludeComponents` to drop unused classes; tunable custom properties (`--as-dropdown-max-w`, `--as-inset`); the `data-dirty` change-tracking hooks (`as-default-field`, `as-collapsible-*`) |
...
| Bundle optimization | [bundle-optimization.md](../atscript-ui/references/bundle-optimization.md) | When asked about JS/CSS bundle size: extractor match channels, `componentCompanions` expansion, `excludeComponents` semantics, per-branch shortcut mounting, prebuilt-CSS granularity (SSOT lives in the root `atscript-ui` skill)                                                                                                                                                                                                                                                                                                                                                              |
