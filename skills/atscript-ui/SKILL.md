...
| 5   | **Value-help is lazy.** `extractValueHelp(prop)` returns `ValueHelpInfo \| undefined` synchronously (since 0.1.134 it follows reference chains to the first `@db.rel.FK` link, so projected view fields keep their dictionary); the actual options/rows fetch happens later via `resolveValueHelp(url)` and is cached per URL across the app. Use `resetValueHelpCache()` to force a re-fetch.                                                                                                                                                                                                                                                                                                                                                                              |
