...
| -------------------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| **FormDef**                            | `{ type, rootField, fields, flatMap }` — flattened form layout from a `.as` type. Built once by `createFormDef(type)`.                                                                                                                                                                          |
...
| **TableDef**                           | `{ type, columns, flatMap, primaryKeys, preferredId, versionColumn?, crud, canRemove, actions, searchable, ... }` — built from a moost-db `/meta` response via `createTableDef(meta)`. `versionColumn` is present on OCC-protected tables (`@db.column.version`) and is skipped from `columns`. |
...
| **MetaResponse**                       | Shape of moost-db `/meta` endpoint response — `{ searchable, primaryKeys, preferredId, versionColumn?, crud, actions, relations, fields, type }`. `versionColumn` is present when the table opts into OCC via `@db.column.version`.                                                             |
...
| **TableStateData / TableStateMethods** | Framework-agnostic contract a Vue/React wrapper must satisfy; covers reactive arrays + mutators.                                                                                                                                                                                                |
