...
| `@ui.table.selectWith "path"`    | `UI_TABLE_SELECT_WITH` | `path: string`                | Extra sibling leaf path added to `$select` when this column is displayed; never rendered as its own column / toggle / config row; **multiple, appended**. See the `atscript-ui-tables` skill (cells → fetching sibling fields).                   |
| `@ui.table.exclude`              | `UI_TABLE_EXCLUDE`     | (none)                        | Completely removes the field from the table — never a column, not displayable/filterable/sortable, absent from the config dialog. Stays in the type and remains a valid `@ui.table.selectWith` target, so its data can still ride in the payload. |
...
`nodeType: ["interface", "type"]` — these sit on the **model**, not on a prop, and are not inherited through references (`passedWhenReferred: false`). Read by `buildModelRoutes(models)` from `@atscript/ui`, which derives a sorted route registry (`TModelRoute[]`) from a model list — typically the `atscriptModels` manifest generated by `dbPlugin({ manifest: "src/models.gen.ts" })` in `atscript.config`. Consumers map the registry to their router + side-nav once; every new `@db.table` / `@db.view` model appears automatically.

| Annotation             | Constant        | Arguments       | What it controls                                                                                                                     |
| ---------------------- | --------------- | --------------- | ------------------------------------------------------------------------------------------------------------------------------------ |
| `@ui.nav.group 'name'` | `UI_NAV_GROUP`  | `group: string` | Nav section (`route.group`). Models sharing a value belong to one section; grouping/rendering is the consumer's job.                 |
| `@ui.nav.order n`      | `UI_NAV_ORDER`  | `order: number` | Sort position (`route.order`) — ascending, undefined last, stable ties.                                                              |
| `@ui.nav.hidden`       | `UI_NAV_HIDDEN` | (none)          | Opt-out for API-only models. Route is **still returned** with `hidden: true` — the consumer filters (unfiltered `v-for` renders it). |

`buildModelRoutes` contract: only `@db.table` / `@db.view` / `@db.view.for` models are included (others skipped silently); path precedence `@db.http.path` > string `@db.table`/`@db.view` name > type id as-is (mirrors moost-db's controller prefix resolution, so the path matches a token-bound `@TableController(Model)` mount), normalized without leading/trailing slashes (empty → omitted); label from `@meta.label` else humanized last path segment; `kind` is `'view'` for views, `'table'` otherwise. Don't re-sort the result — input already carries the stable order tiebreak.
...
Full pipeline + router/nav mapping examples: https://ui.atscript.dev/tables/model-routes
...
| --------------------------------------- | -------------------------------------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `@db.rel.FK "TargetType"`               | `DB_REL_FK`                                        | Foreign-key marker. Drives `extractValueHelp(prop)` → `ValueHelpInfo`; since 0.1.134 the marker may sit anywhere along a reference chain (view field → source column → dictionary).                                                                                                                                                                                                                                                                                                                                                                                                                                                                        |
...
| `@db.column.version`                    | (no const — read via `MetaResponse.versionColumn`) | Marks one `number.int` field as the server-managed OCC version column. SQL adapter auto-creates with `NOT NULL DEFAULT 0` and auto-bumps on every update; the column name surfaces as `meta.versionColumn`. `createFormDef(type, { versionColumn })` excludes the prop from `fields[]` (so renderers don't paint it) while keeping the value in `flatMap` + form data for the server's `$cas` lift. `createTableDef` skips it from `def.columns`; `resolveValueHelp` skips it from `primaryKeys` / `filterableFields` / `sortableFields` / `attrFields`. See the `atscript-db` skill (OCC reference) and the `atscript-ui-forms` skill's OCC edit pattern. |
