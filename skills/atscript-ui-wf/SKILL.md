...
| 13  | **`<AsWfForm>` exposes host-fired actions.** A host fires a workflow action via a component `ref` or the default-slot `actions` bag: `action(name, data?)` (auto-classifies `@wf.action.withData` vs plain `@ui.form.action`) and `supportsAction(name)`. Gate the host affordance on `supportsAction` — keyed on the declared action **id** (`@ui.form.action` id ∪ `@wf.action.withData` value), never the field name. Canonical use: a dialog's own Cancel button firing a hidden-but-whitelisted `cancel`. Custom shells classify via the public `getDeclaredFormActions(def)` from `@atscript/ui`. See [client.md](references/client.md).                                                                                                  |
...
| Client            | [client.md](references/client.md)                   | `<AsWfForm>` props/emits/slots (`@finished`, `@error`, `@form`, `@submit`, `@loading`; slots `#wf.loading` — default is an `as-form-overlay` icon on a `min-h-[100px]` wrapper, `#wf.error`, `#wf.finished`, `#form.*`), `useWfForm(options)` composable (`start` / `submit` / `action` / `actionWithData` / `retry`), custom `fetch` for auth headers, host-fired actions (`action` / `supportsAction`) |
...
| Recipes           | [recipes.md](references/recipes.md)                 | When assembling a complete flow and needing the load-bearing wiring: login + MFA conditional branch, signup with inline OTP + mid-handler uniqueness errors, invite via email magic link (durable state, `initialToken` resume), checkout with save-draft action-with-data                                                                                                                               |
