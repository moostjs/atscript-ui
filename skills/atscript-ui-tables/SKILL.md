...
description: >-
  ...
  `:display-columns`; JSON-column filters (`filterOps`); virtualization and
  block fetching. Out of scope: forms (`atscript-ui-forms`), workflow forms
...
| 19 | **URL → filter decoding never approximates (0.1.139+).** `$in`/`$nin`/same-field `$or`/invertible `$not` map exactly; a piece the per-field model can't hold (cross-field OR, unknown operator, second range on a field) is left out whole — result is broader, never different — and reported via `<AsTableRoot @unsupported-filter>` / `useTable({ onUnsupportedFilter })` / dev `console.warn` (`urlQueryStringToState` returns them as `unsupported`, never warns). Surface it to the user; don't invent a residual filter. ≤ 0.1.138 it silently reshaped/dropped them. See [filtering.md](references/filtering.md#decoding-a-uniquery-filter). |
