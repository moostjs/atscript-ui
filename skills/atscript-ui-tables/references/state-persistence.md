...
| 7   | **URL filters the per-field model can't hold are carried, not dropped (0.1.140+)** — cross-field OR, second range on one field, negated range become `state.residualFilters` ("Custom filter" chips), written back into every URL in uniqu grammar (`customerId=2&((status=shipped&total>500)^(status=pending&total<=50))&$snapshot`), so reload/share/Back keep them. Still dropped + reported via `<AsTableRoot @unsupported-filter>` / `useTable({ onUnsupportedFilter })` / dev `console.warn`: pieces touching an unknown or `local` column, or everything with `urlQuerySync: { residual: false }` (= 0.1.139). Hydration does not rewrite the URL; the next change does. See [filtering.md](filtering.md#residual-filter-conditions). |
