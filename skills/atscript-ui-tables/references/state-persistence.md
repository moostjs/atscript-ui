...
| 3   | **A marker-less URL overlays the boot baseline, on every pass** (app deep link "open view X, narrowed by Y"). It owns every path it mentions — in a field filter or (0.1.140+) inside a residual condition — so the baseline's say on those paths goes; otherwise it can only add to the baseline — to drop a baseline filter, link with the marker. With `snapshot: false` the table's own URLs are marker-less too, so they overlay as well.                                                                                                                                                                                                                                                                                                                                                                                                                                                   |
...
| 7   | **URL filters the per-field model can't hold are carried, not dropped (0.1.140+)** — cross-field OR, second range on one field, negated range become `state.residualFilters` ("Custom filter" chips), written back into every URL in uniqu grammar (`customerId=2&((status=shipped&total>500)^(status=pending&total<=50))&$snapshot`), so reload/share/Back keep them. Still dropped + reported via `<AsTableRoot @unsupported-filter>` / `useTable({ onUnsupportedFilter })` / dev `console.warn`: a piece correlating a server column with a `local` one, or everything with `urlQuerySync: { residual: false }` (= 0.1.139). Pieces and `$sort` entries on fields the caller cannot read → `@fields-dropped` instead (0.1.141+, [hidden-fields.md](hidden-fields.md)). Hydration does not rewrite the URL; the next change does. See [filtering.md](filtering.md#residual-filter-conditions). |

Since 0.1.133 the bridge owns **only what the parser consumes**: a key is table-owned iff `urlQueryStringToState` would read it (`urlQueryConsumesKey`, exported from `@atscript/ui-table`) or the bridge has written it before — so read and write agree in both directions. Each write merges into `route.query`; host-owned keys are preserved in place. Consumed without a prior write: `$sort`, `$search`, `$relevance`, `$skip`, `$snapshot` (0.1.139+), and operator-bearing filter keys (`total>100`; 0.1.140+ also keys with `( ) ^ { }` — groups, ORs, `$in` lists). The bridge splits on top-level `&` only — a group's inner `&` stays inside one key (0.1.140+). NOT consumed, therefore never removed: `$limit`, any other `$key`, and a plain `field=value` key already present at mount (indistinguishable from a host flag until the bridge writes it itself).
...
// { filters, sorters, skip?, searchTerm, snapshot?: true, unsupported?: UnsupportedFilter[], residual?: FilterExpr[],
//   unknown?: FilterExpr[], unknownSorters?: SortControl[] }
```

Both honour `UrlQuerySync` symmetrically. The encoder reuses `buildTableQuery` for the filter/sort/search shape and appends `$skip` / `$limit` for pagination. The decoder produces a `UrlQueryStateSnapshot` (`skip` is the raw record offset — consumer computes `page = floor(skip / itemsPerPage) + 1`; `snapshot: true` means clear every gate-owned filter/sorter before applying — use `gateOwns`; `unsupported` (0.1.139+) lists left-out filter pieces — the parser never warns, report them yourself; `residual` (0.1.140+) holds the carried pieces — `unsupported` then lists only the lost ones — pass them to the encoder as `residualFilters`; `sync.residual: false` disables both directions; `unknown` / `unknownSorters` (0.1.141+) hold what named a field outside `knownFields` — a bare `field=value` is not listed, it may be a host flag).
...
| `activeSnapshot` | `ComputedRef<PresetSnapshot>`              | Dirty baseline — pruned of fields the caller cannot read (0.1.141+).                 |
| `isDirty`        | `ComputedRef<boolean>`                     | Unsaved indicator.                                                                   |
| `droppedFields`  | `ComputedRef<DroppedFieldsReport \| null>` | Quiet "fields not available" note (0.1.141+) — [hidden-fields.md](hidden-fields.md). |
