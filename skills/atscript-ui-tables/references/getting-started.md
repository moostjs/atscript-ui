...
| `alwaysSelected`        | `string[]`                                                         | —              | Leaf paths always added to `$select` (deduped, gated by available meta), regardless of visible columns; additive only (unions into projection — does not AND/restrict like `forceFilters`/`forceSorters`); for headless/custom renderers. See [cells.md](cells.md). |
