...
- `url=` activates the moost-db path; `:query-fn=` activates the custom path; `:rows` + `:columns` activates local mode (no client, no `/meta`). See [query.md](query.md).
...
| ----------------------- | ------------------------------------------------------------------ | -------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `url`                   | `string`                                                           | (required\*)   | moost-db endpoint, e.g. `/api/db/tables/products`. \*Still required even with `queryFn` — drives `/meta`. Optional only in local mode (`rows`).                                                                                                                       |
| `rows`                  | `Record<string, unknown>[]`                                        | —              | Local mode (since 0.1.134): render this array, no client and no `/meta`. Reactive — replacing it re-queries locally. See [query.md](query.md#path-c-local-rows-since-01134).                                                                                          |
| `columns`               | `ColumnDef[]`                                                      | —              | Local mode: the columns to render. Read once at setup.                                                                                                                                                                                                                |
| `searchPaths`           | `string[]`                                                         | —              | Local mode: paths `searchTerm` matches (substring, case-insensitive). Omit to disable search.                                                                                                                                                                         |
| `localSort`             | `boolean`                                                          | `true`         | Local mode: in-memory sort from the active sorters.                                                                                                                                                                                                                   |
| `displayColumns`        | `DisplayColumnDef[]`                                               | —              | Client-owned columns merged into the column list (since 0.1.134) — hideable, reorderable, preset-persisted, never in `$select`/`$sort`/filters. See [customization.md](customization.md#display-only-columns-displaycolumns-since-01134).                             |
| `rowActions`            | `RowActionsConfig`                                                 | —              | Per-screen row-action policy (since 0.1.134): `include` / `exclude` / `overrides` / `extra`. The server's `$actions` gate still wins. See [actions-selection.md](actions-selection.md#per-screen-row-action-policy-rowactions-since-01134).                           |
...
| `alwaysSelected`        | `string[]`                                                         | —              | Leaf paths always added to `$select` (deduped, gated by available meta), regardless of visible columns; additive only (unions into projection — does not AND/restrict like `forceFilters`/`forceSorters`); for renderless/custom renderers. See [cells.md](cells.md). |
