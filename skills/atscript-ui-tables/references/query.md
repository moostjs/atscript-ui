...
| `applyUrlQuery(urlString, opts?)`                                | Hydrate state from URL string (echo-guarded). `opts.mode` is `"merge"` (default) or `"replace"` — see [state-persistence.md](state-persistence.md).           |
